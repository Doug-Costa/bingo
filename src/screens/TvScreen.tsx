...
const imgBingoLogo = require('../assets/new_theme1/Bingo_logo-removebg-preview.png');
const imgHeaderMiddle = require('../assets/new_theme1/Bingo_main_Header_middle-removebg-preview.png');
const imgAquiSorteDown = require('../assets/new_theme1/aqui_e_sorte_logo_down-removebg-preview.png');
const imgMoneyPack = require('../assets/new_theme1/Money_pack-removebg-preview.png');
const img4LeafClover = require('../assets/new_theme1/4-leaf-removebg-preview.png');
...
function JackpotTag({ amount, theme, isTema04 }: { amount: number; theme: ThemeTokens; isTema04?: boolean }) {
  ...
  useEffect(() => {
    if (isTema04) {
      ...
      Animated.loop(
        Animated.sequence([
          Animated.timing(pulseGlow, { toValue: 0.9, duration: 1000, useNativeDriver: true }),
          Animated.timing(pulseGlow, { toValue: 0.4, duration: 1000, useNativeDriver: true }),
        ])
      ).start();
    }
  }, [isTema04]);
    ...
    return (
      <Animated.View style={[
        styles.jackpotTag,
        {
          ...
          borderWidth: 2,
          backgroundColor: 'rgba(0, 24, 76, 0.85)',
        ...
        <LinearGradient
          colors={['rgba(0, 212, 255, 0.1)', 'rgba(0, 212, 255, 0.25)', 'rgba(0, 212, 255, 0.1)']}
          start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
          style={[styles.jackpotTagGrad, { paddingVertical: 10, paddingHorizontal: 16 }]}
        >
          <Image
            source={imgMoneyPack}
            style={{
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={[styles.jackpotTagLabel, { color: theme.primary, letterSpacing: 2, fontSize: 10, fontWeight: '900' }]}>
              ACUMULADO
            </Text>
            <Text style={[styles.jackpotTagAmount, { color: theme.accent, fontSize: 26, fontWeight: '950', shadowColor: '#000', shadowOffset: { width: 1, height: 1 }, shadowOpacity: 0.8, shadowRadius: 2 }]}>
              {fmtCurrency(amount)}
            </Text>
          </View>
        </LinearGradient>
...
function LobbyView({
  ...
  const isTema04 = getThemeKey(theme) === 'tema04';
  ...
  const CountdownContent = () => (
    <>
      ...
      {showJackpot && effectiveJackpot !== null && (
        <JackpotTag amount={effectiveJackpot} theme={theme} isTema04={isTema04} />
      )}
      ...
      {isTema04 ? (
        <View style={{ width: '100%', marginTop: 12, gap: 8 }}>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {/* Sorteio */}
            <View style={{ flex: 1, backgroundColor: 'rgba(0, 24, 76, 0.6)', borderColor: theme.borderSecondary, borderWidth: 1.5, borderRadius: 12, padding: 8, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Image source={img4LeafClover} style={{ width: 22, height: 22, resizeMode: 'contain' }} />
              <View>
                <Text style={{ fontSize: 9, fontWeight: '900', color: theme.textSecondary, letterSpacing: 1 }}>SORTEIO</Text>
                <Text style={{ fontSize: 15, fontWeight: '900', color: '#ffffff' }}>#{d.incrementalId || '---'}</Text>
              </View>
            </View>

            {/* Doação */}
            <View style={{ flex: 1, backgroundColor: 'rgba(0, 24, 76, 0.6)', borderColor: theme.borderSecondary, borderWidth: 1.5, borderRadius: 12, padding: 8, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <View style={{ backgroundColor: 'rgba(0, 255, 136, 0.15)', width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 12 }}>💙</Text>
              </View>
              <View>
                <Text style={{ fontSize: 9, fontWeight: '900', color: theme.textSecondary, letterSpacing: 1 }}>DOAÇÃO</Text>
                <Text style={{ fontSize: 15, fontWeight: '900', color: theme.secondary }}>R$ 0,20</Text>
              </View>
            </View>
          </View>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {/* Data */}
            <View style={{ flex: 1, backgroundColor: 'rgba(0, 24, 76, 0.6)', borderColor: theme.borderSecondary, borderWidth: 1.5, borderRadius: 12, padding: 8, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <View style={{ backgroundColor: 'rgba(0, 212, 255, 0.15)', width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }}>
                <Icon name="calendar" size={12} color={theme.primary} />
              </View>
              <View>
                <Text style={{ fontSize: 9, fontWeight: '900', color: theme.textSecondary, letterSpacing: 1 }}>DATA</Text>
                <Text style={{ fontSize: 14, fontWeight: '900', color: '#ffffff' }}>{new Date(d.scheduledAt || Date.now()).toLocaleDateString('pt-BR')}</Text>
              </View>
            </View>
            ...
            <View style={{ flex: 1, backgroundColor: 'rgba(0, 24, 76, 0.6)', borderColor: theme.borderSecondary, borderWidth: 1.5, borderRadius: 12, padding: 8, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <View>
                <Text style={{ fontSize: 9, fontWeight: '900', color: theme.textSecondary, letterSpacing: 1 }}>HORA</Text>
                <Text style={{ fontSize: 14, fontWeight: '900', color: '#ffffff' }}>{fmtTime(d.scheduledAt)}</Text>
              </View>
            </View>
          </View>
        </View>
...
// ─── Banner Aqui Sorte ──────────────────────────────────────────────────────────
function AquiSorteBanner({ theme }: { theme: ThemeTokens }) {
  const rotateAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    Animated.loop(
      Animated.sequence([
        Animated.timing(rotateAnim, { toValue: 1, duration: 2200, useNativeDriver: true }),
        Animated.timing(rotateAnim, { toValue: -1, duration: 2200, useNativeDriver: true }),
      ])
    ).start();
  }, [rotateAnim]);
    ...
    <View style={isLandscape ? styles.inGameLandscapeWrapper : styles.inGamePortraitWrapper}>
      ...
      <View style={styles.inGameMainColumn}>
        ...
        <View style={[styles.sectionPrizesJackpotRow, { borderColor: theme.borderSecondary, backgroundColor: theme.glassBg }]}>
          ...
          {themeName === 'tema04' ? (
            <View style={{ flex: 1.2 }}>
              <JackpotTag amount={isJackpotActive ? (jackpotAmount || 0) : 0} theme={theme} isTema04={true} />
            </View>
          ) : (
            <View
              style={[
      ...
      <View style={styles.inGameVerticalRightColumn}>
        ...
        {/* Banner Aqui Sorte no Rodapé */}
        {themeName === 'tema04' && (
          <AquiSorteBanner theme={theme} />
        )}
      </View>
    </View>
  );
...
function TvHeader({
  ...
  const isTema04 = getThemeKey(t) === 'tema04';
  ...
  useEffect(() => {
    if (isTema04) {
      Animated.loop(
        Animated.sequence([
          Animated.timing(logoPulse, { toValue: 1.02, duration: 1200, useNativeDriver: true }),
          Animated.timing(logoPulse, { toValue: 1.0, duration: 1200, useNativeDriver: true }),
        ])
      ).start();

      Animated.loop(
        Animated.sequence([
          Animated.timing(centerPulse, { toValue: 1.04, duration: 1800, useNativeDriver: true }),
          Animated.timing(centerPulse, { toValue: 1.0, duration: 1800, useNativeDriver: true }),
        ])
      ).start();
    }
  }, [isTema04, logoPulse, centerPulse]);

  if (isTema04) {
    return (
      <View style={[styles.header, { backgroundColor: theme.headerBg, borderColor: theme.borderSecondary, height: 75, paddingVertical: 4 }]}>
        {/* Esquerda: Logo "BINGO SHOW" + AO VIVO */}
        <View style={[styles.headerLeft, { alignItems: 'center' }]}>
          <Animated.Image
            source={imgBingoLogo}
            style={{ width: 130, height: 48, resizeMode: 'contain', transform: [{ scale: logoPulse }] }}
          />
          <View style={[styles.connRow, { marginLeft: 10 }]}>
            <Animated.View style={[styles.connDot, {
              backgroundColor: connected ? theme.success : '#f59e0b',
              opacity: connected ? 1 : pulse,
            }]} />
            <Text style={[styles.connText, { color: connected ? theme.success : '#f59e0b', fontSize: 10 }]}>
              {connected ? 'AO VIVO' : 'CONECTANDO...'}
            </Text>
          </View>
        </View>

        {/* Centro: Título do Cabeçalho "BINGO AO VIVO" */}
        <View style={styles.headerCenter}>
          <Animated.Image
            source={imgHeaderMiddle}
            style={{ width: 280, height: 60, resizeMode: 'contain', transform: [{ scale: centerPulse }] }}
          />
        </View>

        {/* Direita: Data, Hora, Som e Desconectar */}
        <View style={styles.headerRight}>
          <View style={[styles.headerBadge, {
            borderColor: theme.borderPrimary,
            backgroundColor: 'rgba(0, 24, 76, 0.75)',
            ...
            <Text style={[styles.headerBadgeValue, { color: '#ffffff', fontSize: 13, fontFamily: 'monospace', fontWeight: 'bold' }]}>
              {dateStr}   |   {timeStr}
            </Text>
          </View>
          ...
          {onToggleSound && (
            <TouchableOpacity
              style={[styles.audioBtn, { borderColor: soundOn ? theme.success : theme.borderMuted, backgroundColor: 'rgba(0,0,0,0.3)' }]}
          ...
          <TouchableOpacity style={[styles.logoutBtn, { borderColor: theme.borderMuted, backgroundColor: 'rgba(0,0,0,0.3)' }]} onPress={onLogout}>
